// Llama Service - OpenAI-compatible API wrapper
// Replaces Gemini service with Llama 3.1 8B Instruct
...
class DeepSeekService {
  ...
  async generateResponse(message, context = {}, chatHistory = []) {
    try {
      console.log('🤖 Using Llama 3.1 8B Instruct for response...');
...
- Keep the tone similar to friendly AI study assistants (Gemini, ChatGPT)`;
  ...
  async analyzeImage(imageBase64, context = '') {
    // Llama 3.1 8B doesn't support vision yet, fallback to text extraction
    console.log('⚠️  Image analysis not supported by Llama 3.1 8B, using fallback');
