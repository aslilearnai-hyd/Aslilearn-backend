...
class GeminiService {
  ...
  constructor() {
    this.modelName = resolveAllowedGeminiModel(
      process.env.VIDYA_AI_GEMINI_MODEL || GEMINI_LITE_MODEL,
    );
  }

  async generateEducationalContent(prompt: string, context = ''): Promise<string> {
  ...
  async analyzeImage(imageBase64: string, prompt = 'Describe this educational image.'): Promise<string> {
    if (!apiKey) throw new Error('GEMINI_API_KEY is required');
    const result = await generateContentCompat({
      apiKey,
      model: this.modelName,
