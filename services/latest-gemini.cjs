...
class LatestGeminiService {
  ...
  async initializeGemini() {
    try {
      console.log('🔧 Initializing Latest Gemini API...');
      
      // Try the latest model names and different API versions
      const modelConfigs = [
      ...
      for (const config of modelConfigs) {
        try {
          ...
          console.log(`✅ Gemini API working with model: ${config.name} (${config.version})`);
          console.log(`📝 Test response: ${text.substring(0, 100)}...`);
          break;
        } catch (error) {
          console.log(`❌ Gemini model ${config.name} (${config.version}) failed: ${error.message}`);
        }
      }
...
const latestGeminiService = new LatestGeminiService();

module.exports = { latestGeminiService };
