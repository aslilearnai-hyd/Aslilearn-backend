...
} from '../rag/pdf/activity-title-utils.js';
import {
  activityPatternExtractIsComplete,
  mapActivityRowForToolSlug,
  scoreActivityExtractRow,
} from '../rag/pdf/extract/pdf-activity-extract.js';
...
import { extractToolItemsFromPdfText } from '../rag/pdf/extract/pdf-tool-extract.js';
import {
...
} from '../validation/pdf-extract-validation.js';
...
export async function extractAndGenerateAllItems(toolType, rawPdfText, params = {}) {
  ...
  if (toolType === 'activity-project-generator' || toolType === 'project-idea-lab') {
    ...
    if (workbookActivities && workbookActivities.length > 0) {
      const { canonicalizeActivityExtractedItem } = await import('../generators/core/ai-content-engine-service.js');
  ...
  if (toolType === 'worksheet-mcq-generator' && extractedItems.length) {
    ...
    const { canonicalizeWorksheetExtractedItem } = await import('../generators/core/ai-content-engine-service.js');
