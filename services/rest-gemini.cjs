...
class RestGeminiService {
  ...
  async initializeGemini() {
    try {
      console.log('🔧 Initializing Gemini REST API...');
      ...
      if (response.ok) {
        ...
        console.log('✅ Gemini REST API working');
        console.log('📋 Available models:', data.models?.map(m => m.name).slice(0, 5));
        this.isAvailable = true;
      } else {
        console.log('❌ Gemini REST API failed:', response.status, response.statusText);
        const errorText = await response.text();
        console.log('Error details:', errorText);
      }
    } catch (error) {
      console.log('⚠️ Gemini REST API initialization failed:', error.message);
    }
  }

  async generateResponse(message, context = {}, chatHistory = []) {
    // Try Gemini REST API first if available
    if (this.isAvailable) {
      try {
        console.log('🤖 Using Gemini REST API for response...');
        return await this.generateGeminiResponse(message, context, chatHistory);
      } catch (error) {
        console.log('❌ Gemini REST API failed, falling back to enhanced service:', error.message);
        this.isAvailable = false;
      }
    }
...
- Be encouraging and supportive`;
      ...
      // Use the REST API with correct model name
...
const restGeminiService = new RestGeminiService();

module.exports = { restGeminiService };
